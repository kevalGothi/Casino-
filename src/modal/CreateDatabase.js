...
const CreateTrx = async(req, res) => {
    ...
    // Reset DataBase Trx
    await connection.execute('DELETE FROM trx');

    let arr = ['trx10', 'trx5', 'trx3', 'trx'];

    for (let i = 0; i < arr.length; i++) {
        const sql = "INSERT INTO trx SET period = ?, game = ?, amount = 6, status = 1, time = ?";
        await connection.execute(sql, ['2022070110000', arr[i], timeNow]);
        const sql_1 = "INSERT INTO trx SET period = ?, game = ?, amount = 0, status = 0, time = ?";
        await connection.execute(sql_1, ['2022070110001', arr[i], timeNow]);
    }
    console.log("Create Success Database Trx.");
}
