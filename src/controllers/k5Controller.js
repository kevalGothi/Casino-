...
const rosesPlus = async (auth, money) => {
    ...
    if (money >= 10000) {
        if (f1.length > 0) {
            ...
            await connection.query('UPDATE users SET winning = winning + ?, roses_f1 = roses_f1 + ?, roses_f = roses_f + ?, roses_today = roses_today + ? WHERE phone = ? ', [rosesF1, rosesF1, rosesF1, rosesF1, infoF1.phone]);
            ...
            if (f2.length > 0) {
                ...
                await connection.query('UPDATE users SET winning = winning + ?, roses_f = roses_f + ?, roses_today = roses_today + ? WHERE phone = ? ', [rosesF2, rosesF2, rosesF2, infoF2.phone]);
                ...
                if (f3.length > 0) {
                    ...
                    await connection.query('UPDATE users SET winning = winning + ?, roses_f = roses_f + ?, roses_today = roses_today + ? WHERE phone = ? ', [rosesF3, rosesF3, rosesF3, infoF3.phone]);
                    ...
                    if (f4.length > 0) {
                        ...
                        await connection.query('UPDATE users SET winning = winning + ?, roses_f = roses_f + ?, roses_today = roses_today + ? WHERE phone = ? ', [rosesF4, rosesF4, rosesF4, infoF4.phone]);
                    }
                }
            }
...
const [balance_Avalabel] = await connection.query('SELECT * FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
const TatalUserBalance = Number(balance_Avalabel[0].winning)
return res.status(200).json({
                message: 'Successful bet',
                status: true,
                // data: result,
                change: users[0].level,
                money: TatalUserBalance,
            });
        } else {
            return res.status(200).json({
...
const handling5D = async(typeid) => {
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        await connection.execute('UPDATE `withdrawl_money` SET `amount` = `amount` + ? WHERE `phone` = ? ', [nhan_duoc, phone]);
        const sql = 'UPDATE `users` SET `winning` = `winning` + ? WHERE `phone` = ? ';
