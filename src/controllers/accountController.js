    ...
    try {
        ...
        if (rows.length == 1) {
            if (rows[0].status == 1) {
                ...
                //await connection.execute('INSERT INTO login_notification SET phone = ?', [username]);
                return res.status(200).json({
...
const register = async (req, res) => {
    ...
    try {
        ...
        const [bonusAmount] = await connection.query('SELECT bonus from admin')
        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            ...
            if (check_i.length == 1) {                
                if (check_ip.length <= 6) {
                    ...
                    const sql = "INSERT INTO users SET id_user = ?,phone = ?,name_user = ?,password = ?, plain_password = ?, money = ?, winning = ?, code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?";
                    // await connection.execute(sql, [id_user, username, name_user, md5(pwd), pwd, 0, code, invitecode, ctv, 1, otp2, ip, 1, time]);
                    await connection.execute(sql, [id_user, username, name_user, md5(pwd), pwd, bonusAmount[0].bonus, bonusAmount[0].bonus, code, invitecode, ctv, 1, otp2, ip, 1, time]);
                    await connection.execute('INSERT INTO point_list SET phone = ?', [username]);
                    const TranstionQuery =
            "INSERT INTO transfer_money SET phone = ?, amount = ?, type = ?, status = 1 ";
...
if (rows.length == 0) {
    ...
    console.log(user)
                ...
                await connection.execute("UPDATE users SET password = ?, plain_password = ? WHERE phone = ? ", [md5(pwd), pwd, username]);
                return res.status(200).json({
