...
//             // await connection.execute('UPDATE `users` SET `winning` = `winning` - ?, money = money - ? WHERE `token` = ? ', [total, total,  auth]);
...
const updateUserMoney = async (userId, bonus, level) => {
    try {
        ...
        // Logging the transaction
        // let trans = 'INSERT INTO transfer_money SET amount = ?, phone = ?, status = 3';
        // await connection.query(trans, [bonus, userId, "Bet Commision"]);
        let trans =
            "INSERT INTO betting_commission SET phone = ?, amount = ?,sender_phone=?,type=?, status = ?";
          await connection.query(trans, [
            userId,
